...
router.post('/bfhl', async (req, res) => {
  try {
    ...
    const presentKeys = allowedKeys.filter(k => Object.prototype.hasOwnProperty.call(req.body, k));
    if (presentKeys.length !== 1) {
      return res.status(400).json({ is_success: false, official_email: email, error: 'Request must contain exactly one of: fibonacci, prime, lcm, hcf, AI' });
    }
